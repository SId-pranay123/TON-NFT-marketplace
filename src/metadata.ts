...
export async function uploadFolderToIPFS(folderPath: string): Promise<string> {
    ...
    console.log(`Uploading ${folderPath} to IPFS...`);
    try {
      const response = await pinata.pinFromFS(folderPath);
      console.log(response);
      return response.IpfsHash;
    } catch (error) {
      console.error('An error occurred while pinning the folder:', error);
    }
    // console.log(`Successfully uploadeddddd ${folderPath} to ipfs: https://gateway.pinata.cloud/ipfs/${response.IpfsHash}`);
